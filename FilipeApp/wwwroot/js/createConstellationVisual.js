...
function createConstellationVisual() {
    ...
    let types = ["element", "transformation", "ngo", "distortion", "photograph", "camera", "point3D", "point2D"]
    ...
    const degree_scale = d3.scaleLinear() //linear on purpose
        .domain([0,10,250])
        .range([1,4,100])
    // //The radius that belongs to the point2D nodes based on their ICH element degree
    ...
    chart.initializeSimulation = (callback) => {
        ...
        //Run the first simulation that only places the elements and nodes with an edge weight of 3 + distortion
        //Takes a long time
    ...
    chart.createChart = function(callback) {
        ...
        nodes.forEach(d => {
            if(d.community > -1 && (d.type === "element" || d.type === "photograph" || d.type === "distortion"))
        ...
        edges_primary = edges.filter(d => d.weight === primary_edge_weight || testTypeId(d,"distortion")|| testTypeId(d,"photograph"))
    ...
    function filterNodeData() {
        ...
        nodes = nodes.filter(d => d.type !== "ngo" && d.type !== "project_") //piotr add _ to avoid project elimination 
        //Filter out any node that is a concept that isn't of group: main, unesco or nature
        //Filter out any casestudy, camera, and SDG type nodes as well
        nodes = nodes.filter(d => {
            if(d.type === "photograph") {
                if(d.group === "main" || d.group === "unesco" || d.group === "nature") return true  // piotr need doing something with group
                else return false
            } else if(d.type === "photograph" || d.type === "camera_" || d.type === "sdg") { // piotr add _
                return false
            } else return true
        })//filter
    }//function filterNodeData
    ...
    function filterUnconnectedNodes() {
        ...
        //////////// Remove unconnected point3Ds and regions //////////
        ...
        //Filter out any point3Ds that are not connected to any remaining elements
        let connected_ids = nodes
            .filter(d => d.type === "element")
            .map(d => linked_to_id[d.id].filter(n => n.type === "point3D").map(n => n.id))
        connected_ids = [...new Set([].concat.apply([], connected_ids))]
        //Filter the non-connected ones out
        nodes = nodes.filter(d => {
            if(d.type === "point3D") return connected_ids.indexOf(d.id) >= 0 ? true : false
            else return true
        })//filter
    ...
    function calculateNodeSettings() {
        ...
        nodes.forEach(d => {
            ...
            if(!d.degree) {
                ...
                // if(d.type === "point2D") {
                //     //Find the number of elements connected to this point2D
                //     let node_list = []
                //     regionNodes(d, node_list)
                //     //Filter out all regions and point3Ds
            ...
            //Get all connected point3Ds for an ICH element
            if(d.type === "element" && linked_to_id[d.id] !== null && linked_to_id[d.id] !== undefined) d.point3Ds = linked_to_id[d.id].filter(l => l.type === "point3D").map(l => l.label)

            //Scale the concepts based on their degree
            if(d.type === "photograph") d.r = 50// d.r = degree_scale(d.degree) //@piotr
            else if(d.type === "point2D") d.r = region_radius //region_degree_scale(d.degree)
            ...
            // img1
            if((d.type === "camera" || d.type === "distortion"|| d.type === "photograph") && d.degree > 0 && d.metadata && d.metadata.icon && d.metadata.icon.small !== ""){
                d.img = new Image
                d.img.src = d.metadata.icon.small
                d.img_loaded = false
    ...
    //     //Find the nodes connected to this node for which n is a target and the source is a point2D, point3D or element
    //     let connected_nodes = nodes.filter(d => edge_by_id[d.id + "," + n.id] && (d.type === "point2D" || d.type === "point3D" || d.type === "element"))
    ...
    function prepareEdgeData() {
        ...
        edges.forEach(d => {
            ...
            if(d.weight === primary_edge_weight || testType(d,"distortion")|| testType(d,"photograph")) {
				if (testType(d,"distortion"))
                  d.focus = testType(d,"distortion") ? "secondary" : "primary"
			    else 
				  d.focus = testType(d,"photograph") ? "secondary" : "primary"
    ...
    function createPrimaryData() {
        ...
        edges_primary = edges.filter(d => d.weight === primary_edge_weight || testType(d,"distortion"))
    ...
    function zoomChart() {
        ...
        nodes.forEach(d => {
            ...
            if(d.type === "element" || d.type === "distortion" || d.type === "photograph"  ) d.r *= zoom_node_increase(transform.k)
            //Update the stroke weight to appear to get bigger on zoom in, except for the elements
            if(d.type !== "element" || d.type === "distortion"  || d.type === "photograph" ) d.stroke_width = d.stroke_width_fixed * transform.k
            ...
            if((d.type === "photograph"  || d.type === "distortion") && d.r > 10 && font_size > 6) {
                // let old = font_size
    ...
    function setSelection(node) {
        ...
        if(node.type !== "point2D" || click_active) {
        ...
        } else { //For regions look all the way down to the point3Ds and elements
            edges_selected = []
            nodes_selected = [node]
            //Go into a recursive function that searches the regions, point3Ds and
            //then to the elements connected to the point3Ds
            connectedNodes(node)
        }//else
    ...
    function connectedNodes(n) {
        //Save the edges for which n is a target and the source is a point2D, point3D or element
        let connected_edges = edges.filter(d => d.target === n && (d.source.type === "point2D" || d.source.type === "point3D" || d.source.type === "element" || d.type === "distortion"))
        //https://stackoverflow.com/questions/1374126
        //Save in array for drawing
        edges_selected.push.apply(edges_selected, connected_edges)

        //Find the nodes connected to this node for which n is a target and the source is a point2D, point3D or element
        let connected_nodes = nodes.filter(d => edge_by_id[d.id + "," + n.id] && (d.type === "point2D" || d.type === "point3D" || d.type === "element" || d.type === "distortion"))
        //Save in array for drawing
    ...
    function drawNodes(ctx, d, r, opacity) {
        ...
        // img2
        if (((d.type === "element" || d.type === "distortion"|| d.type === "photograph" )&& d.img && d.img_loaded) && (r > 10 || show_photos)) {
        ...
        if(d.type === "distortion") {
             //drawDiamond(ctx, d.x, d.y, 1.5*r, 2*r)  // piotr uncomment
             //ctx.fill()
			 drawHexagon(ctx, d.x, d.y, sqrt3*r*2, 4*r)
             ctx.fill()
             ...
             ctx.stroke()
                ...
                drawHexagon(ctx, d.x, d.y, sqrt3*r*3, 3*r)
             ctx.fill()
             ctx.lineWidth = d.stroke_width
             ctx.strokeStyle = "yellow"
             ctx.stroke()
        } else if (d.type === "camera" ) {
             drawNinjaStar(ctx, d.x, d.y, 20*r)
             //ctx.fill()
			 //drawHexagon(ctx, d.x, d.y, sqrt3*r*3, 3*r)
             ctx.fill()
             ctx.lineWidth = d.stroke_width
             ctx.strokeStyle = "red"
             ctx.stroke()
        }  else if (d.type === "casestudy" ) {
             drawNinjaStar(ctx, d.x, d.y, 20*r)
             //ctx.fill()
			  drawHexagon(ctx, d.x, d.y, sqrt3*r, 40*r)
             ctx.fill()
             ctx.lineWidth = d.stroke_width
             ctx.strokeStyle = "blue"
             ctx.stroke()
        } else if (d.type === "point2D") { // piotr uncomment these lines
             drawHexagon(ctx, d.x, d.y, sqrt3*r, 2*r)
             ctx.fill()
             ctx.lineWidth = d.stroke_width
             ctx.strokeStyle = "white"
             ctx.stroke()
        } else if (d.type === "point3D") { // piotr uncomment these lines //val-3D//
             // drawDiamond(ctx, d.x, d.y, 5*r, 5*r)
             drawDottedHoverCircle(d)
        } 
		if (d.type === "photograph") {
            //Bigger nodes are white with a colored stroke
            //Small nodes get the colored fill with no stroke
            //[note][draw]: Dessin des concepts  
            //if(r > 2) {
			...
			if (d.type === "element" || d.type === "distortion" || d.type === "photograph") {
            //If the element circles are big enough, draw images inside
    ...
    function renderNodeLabels(ctx, nodes_to_label, opacity) {
        ...
        nodes_to_label
            ...
            .filter(d => d.type === "photograph" && (d.font_size > 7 || (do_print_run && d.font_size > (3/sf_scale))))
            .forEach(d => {
