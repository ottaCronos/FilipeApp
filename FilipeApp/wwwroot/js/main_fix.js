...
if (focus && !focus.match(/^(element|sdg|camera|photograph)_[0-9]+$/)) focus = false
...
let legend_modal_open = function() {
    if (document.querySelector('.modal-intro') !== null) {
        show_legend_modal = false; // was true @piotr
        intro_modal.close();
    } else {
        legend_modal.open('#legend-modal');
    }
...
let constellationVisual = createConstellationVisual()
    ...
    .modalTypes(["photograph", "camera", "distortion", "transformation"])
	//@piotr adding modal type
    ...
    document.getElementById('menu-csv').onclick = () => {
        ...
        if (link == null) {
            ...
            let relations = get_relations(
                ['vocabulary_ich', 'vocabulary_thesaurus', 'point3D', 'distortion'],
                'element'
            );
            let header = [
                'id',
            ...
            for (element_id in relations) {
                ...
                let point3Ds = [];
                for (country_id in relations[element_id]['dst']['point3D']) {
                    point3D = relations[element_id]['dst']['point3D'][country_id];
                    point3Ds.push(point3D['label']);
                }
                row.push(point3Ds.sort().join(', '));
                let concepts = [[], []];
                for (concept_id in relations[element_id]['dst']['vocabulary_thesaurus']) {
                    photograph = relations[element_id]['dst']['vocabulary_thesaurus'][concept_id];
                    concepts[photograph['weight'] - 2].push(photograph['label']);
                }
                for (concept_id in relations[element_id]['dst']['vocabulary_ich']) {
                    photograph = relations[element_id]['dst']['vocabulary_ich'][concept_id];
                    concepts[photograph['weight'] - 2].push(photograph['label']);
                }
