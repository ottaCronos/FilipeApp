...
function createModal(obj) {
    ...
    if (obj.type == 'photograph') {
        ...
        e.style.display = '';
        // Countries
        e = document.getElementById('modal-node-point3Ds');
        // e.innerHTML = obj.point3Ds.sort().join(", ");
        e.style.display = '';
    }
    ...
    if (obj.type == 'photograph') {
        ...
        e.innerHTML = common_translations[language]['photograph'];
        e.style.display = '';
    }
    ...
    if (obj.type == 'camera') {
        let e = document.getElementById('modal-node-type');
        e.innerHTML = common_translations[language]['camera'];
        e.style.display = '';
    }
    ...
    // Considerations
    if (obj.meta && obj.meta.considerations && (obj.type == 'sdg' || obj.type == 'photograph')) {
        let e = document.getElementById('modal-node-description');
    ...
    if (obj.type == 'sdg') {
        ...
        let lists = ['element', 'photograph', 'camera'];
        ...
        for (list of lists) {
            ...
            if (nodes.length > 0) {
                ...
                for (node of nodes) {
                    ...
                    } else if (list == 'element' || list == 'distortion' || list == "camera" || list == "photograph" ) {
                        if (node[1]['weight'] == 3) cls = 'bold';
                        point3Ds = [];
                        linked_countries = get_relations(['point3D'], 'element', node[0].split('_').slice(-1).pop())
                        for (point3D of Object.values(linked_countries[node[0]].dst.point3D)) {
                            point3Ds.push(point3D.label);
                        }
                        point3Ds = ' (' + point3Ds.sort().join(', ') + ')';
                    } else if (list == 'photograph') {
                        cls = 'bold';
                    }
                    let link = node[1].meta.link;
                    html += '<li class="' + cls + '"><a href="' + link + '" target="_blank">' + node[1].label + '</a>' + point3Ds +'</li>';
                }
                html += '</ul></div>';
            }
        }
    ...
    if (obj.type == 'element_') {  // piotr add _ to not producing the menu
    ...
    // Carousel
    if (obj.type == 'camera' || obj.type == 'photograph') {
